...
export function getHelpContent(topic: HelpTopic | null, language: AppLanguage, context: HelpContext) {
  ...
  if (topic === 'shortcuts') {
    return {
      ...
      lines: [
        ...
        `F2 - ${zh ? '切换到上一个标签页' : 'Switch to previous tab'}`,
        `F3 - ${zh ? '切换到下一个标签页' : 'Switch to next tab'}`,
        ...
        `F4 - ${zh ? '显示或隐藏笔记浏览器' : 'Show or hide note browser'}`,
        `F5 - ${zh ? '循环切换编辑器模式' : 'Cycle editor mode'}`,
        `F6 - ${zh ? '打开/关闭提醒列表' : 'Open/close reminder list'}`,
        `F7 - ${zh ? '切换窗口置顶' : 'Toggle window on top'}`,
        `F8 - ${zh ? '重命名标签页' : 'Rename tab'}`,
        `F9 - ${zh ? '切换日间/夜间模式' : 'Toggle day/night mode'}`,
        `F10 - ${zh ? '切换预览主题' : 'Toggle preview theme'}`,
        `F11 - ${zh ? '切换沉浸式全屏' : 'Toggle immersive fullscreen'}`,
        `F12 - ${zh ? '归档当前标签页' : 'Archive current tab'}`,
        `Ctrl+, - ${zh ? '打开设置' : 'Open settings'}`,
  ...
  if (topic === 'expression') {
    return {
      ...
      lines: zh ? [
        '在编辑模式下，输入一行数学表达式后按 Ctrl+Enter，NeoPad 会在行尾追加计算结果并移动到下一行。',
        '支持 +, -, *, /, %, ^ 和括号，也支持 × 和 ÷ 符号。',
        '示例：899*565-451 按 Ctrl+Enter 后变为 899*565-451 = 507484。',
        '如果行内包含非表达式文字，会尽量计算可识别的前缀部分。',
      ] : [
        'In edit mode, type a math expression on one line and press Ctrl+Enter. NeoPad appends the result, then moves to the next line.',
